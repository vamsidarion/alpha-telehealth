...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_doctor'])) {
    ...
    $sql = "INSERT INTO users (name, email, password, role) VALUES ('$name', '$email', '$password', 'doctor')";
    if ($conn->query($sql)) {
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM users WHERE user_id=$doctor_id AND role='doctor'");
}

// Fetch all doctors
$result = $conn->query("SELECT * FROM users WHERE role='doctor' ORDER BY user_id DESC");
...
<main class="container">
    <h1 style="text-align:center;">Manage Doctors</h1>
                ...
                <?php foreach($doctors as $d): ?>
                    <tr>
                        ...
                        <td class="action-links">
                            <a href="doctors.php?delete=<?= $d['user_id']; ?>" onclick="return confirm('Are you sure you want to delete this doctor?');">Delete</a>
